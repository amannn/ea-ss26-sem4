...
function useElapsedSeconds(isFinished) {
  ...
  useEffect(() => {
    // Second time the effect is called,
    // don't start interval
...
function MemoryCards({theme, onThemeChange}) {
  ...
  const numDrawnCards = flippedCards.length;
  const isEvenTurn = numDrawnCards % 2 === 0;
  const isInvalidGuess =
    isEvenTurn && cards[flippedCards.at(-2)] !== cards[flippedCards.at(-1)];

  // TODO: If isInvalidGuess, mark last two cards drawn in red
